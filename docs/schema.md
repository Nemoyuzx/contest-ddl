...
| `event_type` | enum | `competition`、`conference`、`journal_special_issue`、`hackathon`、`summer_camp`、`pre_admission` |
...
| `abstract_deadline` | ISO 8601/null | 论文摘要注册或提交截止 |
| `submission_deadline` | ISO 8601/null | 作品、材料或论文全文提交截止 |
| `primary_deadline` | ISO 8601 | 用于列表排序的最近未来关键节点；全部结束后取最近的历史节点 |
| `description` | string | 从来源正文清洗得到的纯文本介绍，不保存第三方 HTML |
| `schedule` | object[] | 分阶段时间表，含名称、说明和起止时间（来源提供时） |
| `attachments` | object[] | 活动通知、章程等公开附件的名称与 URL |
| `image_url` | URL/string | 来源提供的活动封面 URL，可为空 |
...
| `status` | enum | `registration_upcoming/open/closed`、`submission_upcoming/open/closed`、`upcoming`、`ongoing`、`ended`、`unknown` |
