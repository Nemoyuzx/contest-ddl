...
def guarded(name: str, url: str, collector: Callable[[], list]) -> SourceResult:
    ...
    try:
        collected = collector()
        events, details = collected if isinstance(collected, tuple) else (collected, {})
        return SourceResult(name=name, ok=True, events=events, fetched_at=iso(), url=url, duration_ms=int((time.monotonic() - started) * 1000), details=details)
